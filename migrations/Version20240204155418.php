 ...
 */
final class Version20240204155418 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__member_contact AS SELECT id, member_id FROM member_contact');
        $this->addSql('DROP TABLE member_contact');
        $this->addSql('CREATE TABLE member_contact (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, member_id INTEGER DEFAULT NULL, type VARCHAR(255) NOT NULL, contact_data VARCHAR(255) NOT NULL, CONSTRAINT FK_F39651AC7597D3FE FOREIGN KEY (member_id) REFERENCES member (id) ON UPDATE NO ACTION ON DELETE NO ACTION NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO member_contact (id, member_id) SELECT id, member_id FROM __temp__member_contact');
        $this->addSql('DROP TABLE __temp__member_contact');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__member_contact AS SELECT id, member_id FROM member_contact');
        $this->addSql('DROP TABLE member_contact');
        $this->addSql('CREATE TABLE member_contact (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, member_id INTEGER DEFAULT NULL, city VARCHAR(255) NOT NULL, address VARCHAR(255) NOT NULL, phone VARCHAR(255) NOT NULL, CONSTRAINT FK_F39651AC7597D3FE FOREIGN KEY (member_id) REFERENCES member (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO member_contact (id, member_id) SELECT id, member_id FROM __temp__member_contact');
        $this->addSql('DROP TABLE __temp__member_contact');
        $this->addSql('CREATE INDEX IDX_F39651AC7597D3FE ON member_contact (member_id)');
    }
